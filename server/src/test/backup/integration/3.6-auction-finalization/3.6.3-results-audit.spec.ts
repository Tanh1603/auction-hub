/**
 * Integration Tests for 3.6.3-5 Auction Results and Audit
 * Test Case IDs: TC-3.6.3-01 to TC-3.6.5-06
 */
...
describe('3.6.3-5 Auction Results and Audit', () => {
  ...
  beforeEach(async () => {
    ...
    await prisma.auctionAuditLog.create({
      data: {
        ...
        details: { status: 'success', winningAmount: 1200000000 },
  ...
  // ============================================
  // 3.6.3 Get Auction Results
  // ============================================
  describe('3.6.3 Get Auction Results', () => {
    it('TC-3.6.3-01: Verify get results for completed auction', async () => {
    ...
    it('TC-3.6.3-02: Winner can view their result', async () => {
      const response = await request(app.getHttpServer())
        .get(`/api/auction-finalization/results/${completedAuction.id}`)
    ...
    it('TC-3.6.3-03: Fail get results for live auction', async () => {
  ...
  // 3.6.4 Audit Logs
  // ============================================
  describe('3.6.4 Audit Logs', () => {
    it('TC-3.6.4-01: Verify Admin views audit logs', async () => {
      const response = await request(app.getHttpServer())
    ...
    it('TC-3.6.4-02: Fail Bidder views audit logs', async () => {
    ...
    it('TC-3.6.4-03: Verify audit log includes action details', async () => {
      const response = await request(app.getHttpServer())
        .get(`/api/auction-finalization/audit-logs/${completedAuction.id}`)
        .set('Authorization', `Bearer ${adminToken}`)
        .expect(200);
  ...
  // ============================================
  // 3.6.5 Status Override
  // ============================================
  describe('3.6.5 Status Override', () => {
    it('TC-3.6.5-01: Verify Admin overrides auction status', async () => {
      const response = await request(app.getHttpServer())
        .post(`/api/auction-finalization/override-status`)
        .set('Authorization', `Bearer ${adminToken}`)
        .send({
          auctionId: completedAuction.id,
          newStatus: 'cancelled',
    ...
    it('TC-3.6.5-02: Fail Bidder overrides status', async () => {
      await request(app.getHttpServer())
        .post(`/api/auction-finalization/override-status`)
        .set('Authorization', `Bearer ${bidderToken}`)
        .send({
    ...
    it('TC-3.6.5-03: Verify override creates audit log', async () => {
      await request(app.getHttpServer())
        .post(`/api/auction-finalization/override-status`)
        .set('Authorization', `Bearer ${adminToken}`)
        .send({
    ...
    it('TC-3.6.5-04: Fail override without reason', async () => {
      const response = await request(app.getHttpServer())
        .post(`/api/auction-finalization/override-status`)
        .set('Authorization', `Bearer ${adminToken}`)
        .send({
