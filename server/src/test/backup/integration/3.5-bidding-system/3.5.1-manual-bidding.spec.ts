/**
 * Integration Tests for 3.5.1 Manual Bidding
 * Test Case IDs: TC-3.5.1-01 to TC-3.5.1-30
 *
...
describe('3.5.1 Manual Bidding', () => {
  ...
  describe('Successful Bids', () => {
    it('TC-3.5.1-01: Verify first bid at starting price', async () => {
    ...
    it('TC-3.5.1-02: Verify bid at minimum increment', async () => {
      // Create initial bid
    ...
    it('TC-3.5.1-03: Verify bid higher than minimum increment', async () => {
      await prisma.auctionBid.create({
        data: {
          auctionId: liveAuction.id,
          participantId: participant1.id,
      ...
      const response = await request(app.getHttpServer())
        .post('/api/manual-bid')
        .set('Authorization', `Bearer ${bidder2Token}`)
        .send({ auctionId: liveAuction.id, amount: 1200000000 })
        .expect(201);
    });

    it('TC-3.5.1-04: Verify self-outbidding (outbid own highest bid)', async () => {
      await prisma.auctionBid.create({
        data: {
          auctionId: liveAuction.id,
          participantId: participant1.id,
  ...
  describe('Bid Validation Failures', () => {
    it('TC-3.5.1-05: Fail with insufficient increment', async () => {
      await prisma.auctionBid.create({
        data: {
          auctionId: liveAuction.id,
          participantId: participant1.id,
      ...
      const response = await request(app.getHttpServer())
        .post('/api/manual-bid')
        .set('Authorization', `Bearer ${bidder2Token}`)
        .send({ auctionId: liveAuction.id, amount: 1040000000 })
        .expect(400);

      expect(response.body.message).toMatch(/increment|minimum/i);
    });

    it('TC-3.5.1-06: Fail with bid below current highest', async () => {
      await prisma.auctionBid.create({
        data: {
          auctionId: liveAuction.id,
          participantId: participant1.id,
    ...
    it('TC-3.5.1-07: Fail with bid below starting price', async () => {
      const response = await request(app.getHttpServer())
        .post('/api/manual-bid')
        .set('Authorization', `Bearer ${bidder1Token}`)
        .send({ auctionId: liveAuction.id, amount: 500000000 })
        .expect(400);
    });

    it('TC-3.5.1-08: Fail with zero amount', async () => {
      const response = await request(app.getHttpServer())
        .post('/api/manual-bid')
        .set('Authorization', `Bearer ${bidder1Token}`)
    ...
    it('TC-3.5.1-09: Fail with negative amount', async () => {
      const response = await request(app.getHttpServer())
        .post('/api/manual-bid')
        .set('Authorization', `Bearer ${bidder1Token}`)
        .send({ auctionId: liveAuction.id, amount: -1000000000 })
    ...
    it('TC-3.5.1-10: Fail with missing auctionId', async () => {
      const response = await request(app.getHttpServer())
        .post('/api/manual-bid')
        .set('Authorization', `Bearer ${bidder1Token}`)
    ...
    it('TC-3.5.1-11: Fail with missing amount', async () => {
      const response = await request(app.getHttpServer())
        .post('/api/manual-bid')
        .set('Authorization', `Bearer ${bidder1Token}`)
  ...
  describe('Participant Status Checks', () => {
    it('TC-3.5.1-12: Fail bid from non-checked-in participant', async () => {
    ...
    it('TC-3.5.1-13: Fail bid from non-registered user', async () => {
    ...
    it('TC-3.5.1-14: Fail bid from withdrawn participant', async () => {
      await prisma.auctionParticipant.update({
        where: { id: participant1.id },
    ...
    it('TC-3.5.1-15: Fail bid from banned user', async () => {
  ...
  describe('Auction Status Checks', () => {
    it('TC-3.5.1-20: Fail bid on scheduled auction', async () => {
      const location = await prisma.location.findFirst();
      ...
      const response = await request(app.getHttpServer())
        .post('/api/manual-bid')
        .set('Authorization', `Bearer ${bidder1Token}`)
        .send({ auctionId: scheduledAuction.id, amount: 1000000000 })
        .expect(400);
    });

    it('TC-3.5.1-21: Fail bid on completed auction', async () => {
      const location = await prisma.location.findFirst();
      ...
      const response = await request(app.getHttpServer())
        .post('/api/manual-bid')
        .set('Authorization', `Bearer ${bidder1Token}`)
        .send({ auctionId: completedAuction.id, amount: 1000000000 })
        .expect(400);
    });
  });
  ...
  describe('Authentication', () => {
    it('TC-3.5.1-25: Fail bid without authentication', async () => {
      const response = await request(app.getHttpServer())
        .post('/api/manual-bid')
        .send({ auctionId: liveAuction.id, amount: 1000000000 })
        .expect(401);
    ...
    it('TC-3.5.1-26: Fail bid with invalid token', async () => {
      const response = await request(app.getHttpServer())
        .post('/api/manual-bid')
