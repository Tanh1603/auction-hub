/**
 * Integration Tests for 3.4.1 Register for Auction
 * Test Case IDs: TC-3.4.1-01 to TC-3.4.1-16
 */
...
describe('3.4.1 Register for Auction', () => {
  ...
  describe('Successful Registration', () => {
    it('TC-3.4.1-01: Verify bidder registers for auction', async () => {
    ...
    it('TC-3.4.1-02: Verify registration creates pending status', async () => {
      const response = await request(app.getHttpServer())
        .post('/api/register-to-bid')
        .set('Authorization', `Bearer ${bidderToken}`)
        .send({ auctionId: openAuction.id })
  ...
  describe('Registration Failures', () => {
    it('TC-3.4.1-03: Fail duplicate registration', async () => {
    ...
    it('TC-3.4.1-04: Fail registration without auth', async () => {
      await request(app.getHttpServer())
        .post('/api/register-to-bid')
        .send({ auctionId: openAuction.id })
        .expect(401);
    ...
    it('TC-3.4.1-05: Fail registration for non-existent auction', async () => {
      await request(app.getHttpServer())
        .post('/api/register-to-bid')
        .set('Authorization', `Bearer ${bidderToken}`)
    ...
    it('TC-3.4.1-06: Fail registration for closed auction', async () => {
      const location = await prisma.location.findFirst();
    ...
    it('TC-3.4.1-07: Fail banned user registers', async () => {
