/**
 * Integration Tests for 3.4.4-5 Submit and Verify Deposit
 * Test Case IDs: TC-3.4.4-01 to TC-3.4.5-04
 */
...
describe('3.4.4-5 Deposit Operations', () => {
  ...
  // ============================================
  // 3.4.4 Submit Deposit
  // ============================================
  describe('3.4.4 Submit Deposit', () => {
    it('TC-3.4.4-01: Verify submit deposit after document verification', async () => {
    ...
    it('TC-3.4.4-02: Fail submit deposit without document verification', async () => {
    ...
    it('TC-3.4.4-03: Fail submit deposit with wrong amount', async () => {
      const response = await request(app.getHttpServer())
        .post('/api/register-to-bid/submit-deposit')
        .set('Authorization', `Bearer ${bidderToken}`)
        .send({
    ...
    it('TC-3.4.4-04: Fail submit deposit for already paid', async () => {
      await prisma.auctionParticipant.update({
        where: { id: registration.id },
        data: {
    ...
    it('TC-3.4.4-05: Fail submit deposit with negative amount', async () => {
      const response = await request(app.getHttpServer())
        .post('/api/register-to-bid/submit-deposit')
        .set('Authorization', `Bearer ${bidderToken}`)
        .send({
  ...
  // ============================================
  describe('3.4.5 Verify Deposit Payment', () => {
    it('TC-3.4.5-01: Verify deposit payment verification', async () => {
    ...
    it('TC-3.4.5-02: Fail verify incomplete payment', async () => {
      const response = await request(app.getHttpServer())
        .post('/api/register-to-bid/verify-deposit-payment')
        .set('Authorization', `Bearer ${bidderToken}`)
        .send({
    ...
    it('TC-3.4.5-03: Fail verify with invalid sessionId', async () => {
      const response = await request(app.getHttpServer())
        .post('/api/register-to-bid/verify-deposit-payment')
        .set('Authorization', `Bearer ${bidderToken}`)
        .send({
