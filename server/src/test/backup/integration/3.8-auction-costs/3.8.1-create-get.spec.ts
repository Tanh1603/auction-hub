/**
 * Integration Tests for 3.8.1-2 Create and Get Auction Costs
 * Test Case IDs: TC-3.8.1-01 to TC-3.8.2-04
 */
...
describe('3.8.1-2 Create and Get Auction Costs', () => {
  ...
  // ============================================
  // 3.8.1 Create Auction Costs
  // ============================================
  describe('3.8.1 Create Auction Costs', () => {
    it('TC-3.8.1-01: Verify create auction costs with all fields', async () => {
    ...
    it('TC-3.8.1-02: Verify upsert updates existing', async () => {
    ...
    it('TC-3.8.1-03: Verify create with otherCosts array', async () => {
      const response = await request(app.getHttpServer())
        .post(`/api/auction-costs/auction/${auction.id}`)
        .set('Authorization', `Bearer ${adminToken}`)
        .send({
    ...
    it('TC-3.8.1-04: Verify create with empty optional fields', async () => {
      await request(app.getHttpServer())
        .post(`/api/auction-costs/auction/${auction.id}`)
        .set('Authorization', `Bearer ${adminToken}`)
    ...
    it('TC-3.8.1-05: Fail create with negative cost', async () => {
      await request(app.getHttpServer())
        .post(`/api/auction-costs/auction/${auction.id}`)
        .set('Authorization', `Bearer ${adminToken}`)
        .send({ advertisingCost: -1000000 })
    ...
    it('TC-3.8.1-06: Fail create for non-existent auction', async () => {
      await request(app.getHttpServer())
    ...
    it('TC-3.8.1-07: Fail create without authentication', async () => {
      await request(app.getHttpServer())
        .post(`/api/auction-costs/auction/${auction.id}`)
        .send({ advertisingCost: 1000000 })
        .expect(401);
    ...
    it('TC-3.8.1-08: Fail Bidder creating costs', async () => {
      await request(app.getHttpServer())
        .post(`/api/auction-costs/auction/${auction.id}`)
  ...
  // ============================================
  describe('3.8.2 Get Auction Costs', () => {
    beforeEach(async () => {
      await prisma.auctionCost.create({
        data: {
          auctionId: auction.id,
    ...
    it('TC-3.8.2-01: Verify get auction costs', async () => {
      const response = await request(app.getHttpServer())
    ...
    it('TC-3.8.2-02: Verify get includes totalCost calculation', async () => {
      const response = await request(app.getHttpServer())
        .get(`/api/auction-costs/auction/${auction.id}`)
        .set('Authorization', `Bearer ${adminToken}`)
        .expect(200);
    ...
    it('TC-3.8.2-03: Return 404 for auction without costs', async () => {
    ...
    it('TC-3.8.2-04: Fail get for non-existent auction', async () => {
      await request(app.getHttpServer())
