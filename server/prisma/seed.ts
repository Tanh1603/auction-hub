...
async function seedAuctions(
  ...
  if (upcomingData) {
    const auctionsData = upcomingData.data.map((item: any) => ({
      ...
      name: item.name,
      propertyOwner: {
        fullName: users.auctioneer?.fullName || 'Nguyễn Lê Tuấn Anh',
        email: users.auctioneer?.email || 'tanh@gm.com',
        phoneNumber: users.auctioneer?.phoneNumber || '123456789',
        identityNumber: users.auctioneer?.identityNumber || '001234567890',
        organization: 'UIT',
      },
    ...
    for (const auctionData of sampleData.data) {
      ...
      const auction = await prisma.auction.create({
        data: {
          ...
          propertyOwner: {
            fullName: (users.auctioneer as any)?.fullName || 'Property Owner',
            email: (users.auctioneer as any)?.email || 'owner@example.com',
            phoneNumber: (users.auctioneer as any)?.phoneNumber || '0123456789',
            identityNumber:
              (users.auctioneer as any)?.identityNumber || '001234567890',
            organization: 'UIT',
          },
