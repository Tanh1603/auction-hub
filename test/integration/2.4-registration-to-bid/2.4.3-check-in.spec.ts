/**
 * Integration Tests for 2.4.3 Check-in
 * Test Case IDs: TC-2.4.3-01 to TC-2.4.3-08
 */
...
describe('2.4.3 Check-in', () => {
  ...
  describe('Successful Check-in', () => {
    it('TC-2.5.1-30: Verify confirmed participant checks in', async () => {
  ...
  describe('Check-in Failures', () => {
    it('TC-2.4.3-04: Fail check-in for non-confirmed participant', async () => {
    ...
    it('TC-2.4.3-08: Fail check-in for non-registered user', async () => {
    ...
    it('TC-2.5.1-08: Fail check-in for scheduled auction', async () => {
      const location = await prisma.location.findFirst();
      ...
      await request(app.getHttpServer())
        ...
        .expect(403); // Check-in window not open - returns 403 Forbidden
    });
  });
});
