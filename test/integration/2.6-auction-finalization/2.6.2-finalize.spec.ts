/**
 * Integration Tests for 2.6.2 Finalize Auction
 * Test Case IDs: TC-2.6.2-01 to TC-2.6.2-10
 */
...
describe('2.6.2 Finalize Auction', () => {
  ...
  beforeEach(async () => {
    ...
    // Participant 2 (Required for minimum participants rule)
    const bidder2 = await createTestUser(prisma, {
      email: 'fin_bidder2@test.com',
      role: UserRole.bidder,
    });
  ...
  describe('Successful Finalization', () => {
    it('TC-2.6.2-01: Verify Admin finalizes auction with bids', async () => {
    ...
    it('TC-2.6.2-02: Verify finalization creates contract', async () => {
    ...
    it('TC-2.6.1-03: Verify finalization without bids marks as failed', async () => {
    ...
    it('TC-2.6.2-02: Verify finalization creates audit log', async () => {
      await request(app.getHttpServer())
        .post('/api/auction-finalization/finalize')
        .set('Authorization', `Bearer ${adminToken}`)
        .send({ auctionId: endedAuction.id })
  ...
  describe('Finalization Failures', () => {
    it('TC-2.6.2-09: Fail Bidder finalizes auction', async () => {
      await request(app.getHttpServer())
        .post('/api/auction-finalization/finalize')
    ...
    it('TC-2.6.2-07: Fail finalize already finalized', async () => {
    ...
    it('TC-2.6.2-08: Fail finalize live auction', async () => {
      await prisma.auction.update({
        where: { id: endedAuction.id },
