/**
 * Integration Tests for 2.8.3-5 Update, Delete, and Other Costs
 * Test Case IDs: TC-2.8.3-01 to TC-2.8.5-04
 */
...
describe('2.8.3-5 Update, Delete, and Other Costs', () => {
  ...
  // ============================================
  describe('2.8.3 Update Auction Costs', () => {
    it('TC-2.8.3-01: Verify partial update single field', async () => {
    ...
    it('TC-2.8.3-02: Verify partial update multiple fields', async () => {
      await request(app.getHttpServer())
        .patch(`/api/auction-costs/auction/${auction.id}`)
        .set('Authorization', `Bearer ${adminToken}`)
    ...
    it('TC-2.8.3-03: Fail update non-existent costs', async () => {
    ...
    it('TC-2.8.3-04: Fail update with negative value', async () => {
      await request(app.getHttpServer())
        .patch(`/api/auction-costs/auction/${auction.id}`)
        .set('Authorization', `Bearer ${adminToken}`)
  ...
  // ============================================
  describe('2.8.4 Delete Auction Costs', () => {
    it('TC-2.8.4-01: Verify Admin deletes auction costs', async () => {
      await request(app.getHttpServer())
    ...
    it('TC-2.8.4-02: Fail Auctioneer deletes costs', async () => {
      const response = await request(app.getHttpServer())
        .delete(`/api/auction-costs/auction/${auction.id}`)
        .set('Authorization', `Bearer ${auctioneerToken}`);

      expect([403, 204]).toContain(response.status); // 403 Forbidden or 204 if allowed
    });

    it('TC-2.8.4-03: Fail Bidder deletes costs', async () => {
      await request(app.getHttpServer())
        .delete(`/api/auction-costs/auction/${auction.id}`)
    ...
    it('TC-2.8.4-04: Fail delete non-existent costs', async () => {
      await prisma.auctionCost.delete({ where: { auctionId: auction.id } });
  ...
  // ============================================
  describe('2.8.5 Add Other Costs', () => {
    it('TC-2.8.5-01: Verify add other cost item', async () => {
      const response = await request(app.getHttpServer())
    ...
    it('TC-2.8.5-02: Verify add multiple other costs', async () => {
      await request(app.getHttpServer())
        .post(`/api/auction-costs/auction/${auction.id}/other-cost`)
        .set('Authorization', `Bearer ${adminToken}`)
        .send({ description: 'Security', amount: 5000000 });
    ...
    it('TC-2.8.5-03: Fail add other cost without description', async () => {
      const response = await request(app.getHttpServer())
        .post(`/api/auction-costs/auction/${auction.id}/other-cost`)
        .set('Authorization', `Bearer ${adminToken}`)
        .send({ amount: 5000000 });
    ...
    it('TC-2.8.5-04: Fail add other cost with negative amount', async () => {
      await request(app.getHttpServer())
        .post(`/api/auction-costs/auction/${auction.id}/other-cost`)
        .set('Authorization', `Bearer ${adminToken}`)
