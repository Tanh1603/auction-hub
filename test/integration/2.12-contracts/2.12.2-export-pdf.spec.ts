/**
 * Integration Tests for 2.12.2 Export Contract PDF
 * Test Case IDs: TC-2.12.2-01 to TC-2.12.2-04
 */
...
describe('2.12.2 Export Contract PDF', () => {
  ...
  describe('PDF Export', () => {
    it('TC-2.12.2-01: Verify export contract PDF Vietnamese', async () => {
    ...
    it('TC-2.12.2-02: Verify export contract PDF English', async () => {
      const response = await request(app.getHttpServer())
    ...
    it('TC-2.12.2-03: Fail PDF export non-party user', async () => {
      const response = await request(app.getHttpServer())
        .get(`/api/contracts/${contract.id}/pdf/vi`)
    ...
    it('TC-2.12.2-04: Fail PDF export invalid language', async () => {
      const response = await request(app.getHttpServer())
        .get(`/api/contracts/${contract.id}/pdf/de`)
        .set('Authorization', `Bearer ${buyerToken}`);

      // Accept 400 (bad request), 200 (defaults to valid lang), or 404 (language not found)
