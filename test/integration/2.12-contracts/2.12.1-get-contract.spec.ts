/**
 * Integration Tests for 2.12.1 Get Contract
 * Test Case IDs: TC-2.12.1-01 to TC-2.12.1-04
 */
...
describe('2.12.1 Get Contract', () => {
  ...
  describe('Contract Access', () => {
    it('TC-2.12.1-01: Verify get contract by ID (buyer)', async () => {
      const response = await request(app.getHttpServer())
        .get(`/api/contracts/${contract.id}`)
        .set('Authorization', `Bearer ${buyerToken}`)
        .expect(200);

      // Handle wrapped response (e.g., { data: { id, ... } } or { contract: { ... } })
      const contractData =
        response.body.data || response.body.contract || response.body;
      expect(contractData.id).toBe(contract.id);
      expect(contractData.buyerUserId).toBe(buyer.id);
    });

    it('TC-2.12.1-02: Verify seller can access contract', async () => {
    ...
    it('TC-2.12.1-03: Fail non-party accessing contract', async () => {
      const response = await request(app.getHttpServer())
        .get(`/api/contracts/${contract.id}`)
    ...
    it('TC-2.12.1-04: Fail access non-existent contract', async () => {
