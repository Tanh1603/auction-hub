/**
 * Integration Tests for 2.3.1 List Auctions
 * Test Case IDs: TC-2.3.1-01 to TC-2.3.1-10
 */
...
describe('2.3.1 List Auctions', () => {
    ...
    await prisma.auction.createMany({
      data: [
        ...
        },
        {
          ...
          viewTime: '9:00-17:00',
          saleFee: new Decimal(500000),
          depositAmountRequired: new Decimal(500000),
  ...
  describe('Public Listing', () => {
    ...
    it('TC-2.4.15-02: Verify filter by status', async () => {
      // API uses AuctionQueryStatus: 'upcoming', 'now', 'completed'
      // 'now' maps to AuctionStatus.live internally
      const response = await request(app.getHttpServer())
        .get('/api/auctions?status=now')
        .expect(200);

      // Response should contain auctions - status might not be returned in list view
      const auctions = response.body.data || response.body;
      expect(Array.isArray(auctions)).toBe(true);
    });

    it('TC-2.11.4-03: Verify filter by assetType', async () => {
      await request(app.getHttpServer())
        .get('/api/auctions?assetType=other_asset')
        .expect(200);
    });

    it('TC-2.11.2-01: Verify search by name', async () => {
      await request(app.getHttpServer())
        .get('/api/auctions?search=Live')
        .expect(200);
    });

    it('TC-2.5.1-01: Verify sort by startingPrice', async () => {
      await request(app.getHttpServer())
