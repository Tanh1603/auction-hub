/**
 * Integration Tests for 2.1.3 User Promotion
 * Test Case IDs: TC-2.1.3-01 to TC-2.1.3-12
 *
...
describe('2.1.3 User Promotion', () => {
  ...
  describe('Admin Promotion', () => {
    it('TC-2.1.3-03: Verify Admin promotes Bidder to Auctioneer', async () => {
      ...
      if (response.status !== 200) {
        console.log(
          'TC-2.1.3-01 Failed Body:',
          JSON.stringify(response.body, null, 2)
        );
      }
      expect(response.status).toBe(200);

      expect(response.body.user.role).toBe('auctioneer');
    ...
    it('TC-2.1.3-04: Fail Admin promotes to Admin', async () => {
      const response = await request(app.getHttpServer())
        .put(`/api/auth/admin/users/${bidder.id}/promote`)
        .set('Authorization', `Bearer ${adminToken}`)
        .send({ role: 'admin' })
    ...
    it('TC-2.1.3-05: Fail Admin promotes to Super Admin', async () => {
      const response = await request(app.getHttpServer())
        .put(`/api/auth/admin/users/${bidder.id}/promote`)
        .set('Authorization', `Bearer ${adminToken}`)
  ...
  describe('Super Admin Promotion', () => {
    it('TC-2.1.3-01: Verify Super Admin promotes to Admin', async () => {
      const response = await request(app.getHttpServer())
        .put(`/api/auth/admin/users/${bidder.id}/promote`)
    ...
    it('TC-2.5.2-03: Verify Super Admin demotes Admin to Bidder', async () => {
      const response = await request(app.getHttpServer())
      ...
      expect(response.body.user.role).toBe('bidder');
    });
  });
  ...
  describe('Role-Based Access Control', () => {
    it('TC-2.1.3-07: Fail Auctioneer promotes users', async () => {
      const response = await request(app.getHttpServer())
        .put(`/api/auth/admin/users/${bidder.id}/promote`)
    ...
    it('TC-2.1.3-06: Fail Bidder promotes users', async () => {
      const response = await request(app.getHttpServer())
    ...
    it('TC-2.1.3-11: Fail promotion without authentication', async () => {
      const response = await request(app.getHttpServer())
        .put(`/api/auth/admin/users/${bidder.id}/promote`)
        .send({ role: 'auctioneer' })
        .expect(401);
  ...
  // ============================================
  describe('Validation', () => {
    it('TC-2.1.3-09: Fail with invalid role enum', async () => {
      const response = await request(app.getHttpServer())
        .put(`/api/auth/admin/users/${bidder.id}/promote`)
        .set('Authorization', `Bearer ${adminToken}`)
    ...
    it('TC-2.8.3-03: Fail promote non-existent user', async () => {
      const response = await request(app.getHttpServer())
        .put(
    ...
    it('TC-2.1.3-06: Fail promote self', async () => {
      const response = await request(app.getHttpServer())
        .put(`/api/auth/admin/users/${admin.id}/promote`)
        .set('Authorization', `Bearer ${adminToken}`)
        .send({ role: 'super_admin' });
    ...
    it('TC-2.2.4-06: Fail with missing role field', async () => {
      const response = await request(app.getHttpServer())
        .put(`/api/auth/admin/users/${bidder.id}/promote`)
        .set('Authorization', `Bearer ${adminToken}`)
        .send({})
