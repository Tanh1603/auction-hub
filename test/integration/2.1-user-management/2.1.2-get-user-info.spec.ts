/**
 * Integration Tests for 2.1.2 Get Current User Info
 * Test Case IDs: TC-2.1.2-01 to TC-2.1.2-08
 *
...
describe('2.1.2 Get Current User Info', () => {
  ...
  describe('Successful Retrieval', () => {
    it('TC-2.1.2-01: Verify authenticated user gets own info', async () => {
    ...
    it('TC-2.1.2-05: Verify response includes required fields', async () => {
      const response = await request(app.getHttpServer())
        .get('/api/auth/me')
        .set('Authorization', `Bearer ${testToken}`)
        .expect(200);
    ...
    it('TC-2.1.2-06: Verify password NOT included in response', async () => {
      const response = await request(app.getHttpServer())
        .get('/api/auth/me')
        .set('Authorization', `Bearer ${testToken}`)
        .expect(200);
    ...
    it('TC-2.4.10-01: Verify Admin user gets admin role in response', async () => {
  ...
  describe('Authentication Failures', () => {
    ...
    it('TC-2.5.1-17: Fail with invalid token format', async () => {
      const response = await request(app.getHttpServer())
        .get('/api/auth/me')
    ...
    it('TC-2.1.2-07: Fail with malformed Authorization header', async () => {
      const response = await request(app.getHttpServer())
        .get('/api/auth/me')
