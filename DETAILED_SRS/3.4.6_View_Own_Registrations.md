...
| :------- | :-------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **(1)**  | **BR-01** | **Searching Rule:**<br>When user navigates to registrations page, system extracts userId from JWT token.<br>System queries data in the table 'AUCTION_PARTICIPANT' in the database (Refer to 'AUCTION_PARTICIPANT' table in 'DB Sheet' file) based on the user ID and auction ID.                            |
| **(4)**  | **BR-02** | **Querying Rule:**<br>System retrieves registration record with all status fields: submittedAt, documentsVerifiedAt, depositPaidAt, checkedInAt, status.<br>Call method `displayRegistrationStatus(registration)`.                                                                                           |
| **(8)**  | **BR-03** | **Displaying Rule:**<br>The system displays a 'MyRegistrationsPage' screen. (Refer to 'MyRegistrationsPage' view in 'View Description' file).<br>Screen shows registration status (e.g., 'Pending Docs', 'Verified', 'Approved', 'Rejected').<br>If no registration found → display 'Not Registered' status. |
